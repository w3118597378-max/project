	...
	<div>
		<div class="center_view">
			<div class="list_search_view">
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('jinjishenqing','新增')">
						新增
					</el-button>
					<el-button class="del_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('jinjishenqing','删除')">
						删除
					</el-button>
				</div>
			</div>
			<el-table
				...
				<el-table-column min-width="140"
					...
					label="球队积分">
					<template #default="scope">
						<span class="score_badge">{{scope.row.qiuduijifen}}</span>
					</template>
				</el-table-column>
				<el-table-column min-width="140"
					:resizable='true'
					...
					label="比赛排名">
					<template #default="scope">
						<span class="ranking_pill">{{scope.row.bisaipaiming}}</span>
					</template>
				</el-table-column>
				<el-table-column min-width="140"
					:resizable='true'
				...
				<el-table-column min-width="160"
					...
					label="晋级赛事">
					<template #default="scope">
						<span class="light_pill">{{scope.row.jinjisaishi}}</span>
					</template>
				</el-table-column>
				<el-table-column min-width="160"
					:resizable='true'
					...
					label="申请时间">
					<template #default="scope">
						<span class="time_pill">{{scope.row.shenqingshijian}}</span>
					</template>
				</el-table-column>
				<el-table-column min-width="140"
					:resizable='true'
					...
					label="队长账号">
					<template #default="scope">
						<span class="secondary_text">{{scope.row.duizhangzhanghao}}</span>
					</template>
				</el-table-column>
				<el-table-column min-width="140"
					:resizable='true'
					...
					label="队长姓名">
					<template #default="scope">
						<span class="secondary_text">{{scope.row.duizhangxingming}}</span>
					</template>
				</el-table-column>
				<el-table-column label="审核回复" min-width="180" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<span class="light_pill reply_pill">{{scope.row.shhf}}</span>
					</template>
				</el-table-column>
				...
				<el-table-column label="审核" v-if="btnAuth('jinjishenqing','审核')" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" class-name="operation-cell" width="300"  :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button class="view_btn" type="info" v-if=" btnAuth('jinjishenqing','查看')" @click="infoClick(scope.row.id)">
							详情
						</el-button>
						<el-button class="edit_btn" type="primary" @click="editClick(scope.row.id,scope.row)" v-if=" btnAuth('jinjishenqing','修改')">
							修改						</el-button>
						<el-button class="del_btn" type="danger" @click="delClick(scope.row.id,scope.row)"  v-if="btnAuth('jinjishenqing','删除')">
							删除						</el-button>
						<el-button class="cross_btn" v-if="btnAuth('jinjishenqing','结果')" type="success" @click="jinjibisaijieguoCrossAddOrUpdateHandler(scope.row,'cross','是','','','')">
							结果
						</el-button>
					</template>
				</el-table-column>
		...
		<jinjibisaijieguoFormModel ref="jinjibisaijieguoFormModelRef" @formModelChange="formModelChange"></jinjibisaijieguoFormModel>
	</div>
</template>
<script setup>
	...
	import jinjibisaijieguoFormModel from '@/views/jinjibisaijieguo/formModel'
	const jinjibisaijieguoFormModelRef = ref(null)
    const jinjibisaijieguoCrossAddOrUpdateHandler = (row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue) => {
