				...
				<div class="page_container">
					<div class="page_header">
						<div class="page_header__left">
							<div class="page_icon">
								<TrendingUp class="page_icon__svg" />
							</div>
							<div class="page_titles">
								<div class="page_title">晋级申请管理</div>
								<div class="page_subtitle">维护晋级申请、审核状态与结果联动</div>
							</div>
						</div>
					</div>
					<div class="search_card list_search_view">
						<el-form :model="searchQuery" class="search_form">
							...
							<div class="search_view">
								<div class="search_label">晋级赛事：</div>
								<div class="search_box">
									<el-input class="search_inp" v-model="searchQuery.jinjisaishi" placeholder="晋级赛事" clearable>
										<template #prefix><Search class="field_icon" /></template>
									</el-input>
								</div>
							</div>
						...
						<div class="btn_view">
							<el-button class="btn_primary" type="success" @click="addClick" v-if="btnAuth('jinjishenqing','新增')">新增</el-button>
							<el-button class="btn_danger" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)" v-if="btnAuth('jinjishenqing','删除')">删除</el-button>
						</div>
					</div>
					...
					<div class="table_card">
						<el-table
							...
							<el-table-column label="操作" class-name="operation-cell" width="300"  :resizable='true' :sortable='false' align="left" header-align="left">
								<template #default="scope">
									<el-button class="btn_secondary" type="info" v-if=" btnAuth('jinjishenqing','查看')" @click.stop="infoClick(scope.row.id)">详情</el-button>
									<el-button class="btn_secondary" type="primary" v-if=" btnAuth('jinjishenqing','修改')" @click.stop="editClick(scope.row.id,scope.row)">修改</el-button>
									<el-button class="btn_danger" type="danger" v-if="btnAuth('jinjishenqing','删除')" @click.stop="delClick(scope.row.id,scope.row)">删除</el-button>
									<el-button class="btn_secondary" v-if="btnAuth('jinjishenqing','结果')" type="success" @click.stop="jinjibisaijieguoCrossAddOrUpdateHandler(scope.row,'cross','是','','','')">结果</el-button>
								</template>
							</el-table-column>
						</el-table>
					</div>
