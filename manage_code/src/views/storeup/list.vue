				...
				<div class="page_container">
					<div class="page_header">
						<div class="page_header__left">
							<div class="page_icon">
								<Bookmark class="page_icon__svg" />
							</div>
							<div class="page_titles">
								<div class="page_title">收藏管理</div>
								<div class="page_subtitle">查看与维护收藏记录</div>
							</div>
						</div>
					</div>
					...
					<div class="search_card list_search_view">
						...
						<div class="btn_view">
							<el-button class="btn_primary" type="success" @click="addClick" v-if="btnAuth('storeup','新增')">新增</el-button>
							<el-button class="btn_danger" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)" v-if="btnAuth('storeup','删除')">删除</el-button>
						</div>
					</div>
					...
					<div class="table_card">
						<el-table
							...
							<el-table-column label="图片" min-width="140" width="120" :resizable='true' :sortable='false' align="left" header-align="left">
								<template #default="scope">
									<div v-if="scope.row.picture" @click.stop>
										<el-image v-if="scope.row.picture.substring(0,4)=='http'" preview-teleported
											:preview-src-list="[scope.row.picture.split(',')[0]]"
											:src="scope.row.picture.split(',')[0]" style="width:64px;height:64px;border-radius:14px"></el-image>
										<el-image v-else preview-teleported
											:preview-src-list="[$config.url+scope.row.picture.split(',')[0]]"
											:src="$config.url+scope.row.picture.split(',')[0]" style="width:64px;height:64px;border-radius:14px">
										</el-image>
									</div>
							...
							<el-table-column label="操作" class-name="operation-cell" width="260"  :resizable='true' :sortable='false' align="left" header-align="left">
								<template #default="scope">
									<el-button class="btn_secondary" type="info" v-if=" btnAuth('storeup','查看')" @click.stop="infoClick(scope.row.id)">详情</el-button>
									<el-button class="btn_secondary" type="primary" v-if=" btnAuth('storeup','修改')" @click.stop="editClick(scope.row.id,scope.row)">修改</el-button>
									<el-button class="btn_danger" type="danger" v-if="btnAuth('storeup','删除')" @click.stop="delClick(scope.row.id,scope.row)">删除</el-button>
								</template>
							</el-table-column>
						</el-table>
					</div>
