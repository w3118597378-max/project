				...
				<div class="page_container">
					<div class="page_header">
						<div class="page_header__left">
							<div class="page_icon">
								<Images class="page_icon__svg" />
							</div>
							<div class="page_titles">
								<div class="page_title">轮播图管理</div>
								<div class="page_subtitle">管理首页轮播图配置</div>
							</div>
						</div>
					</div>
					...
					<div class="table_card">
						<el-table
							...
							<el-table-column label="值" min-width="140" width="120" :resizable='true' :sortable='false' align="left" header-align="left">
								<template #default="scope">
									<div v-if="scope.row.value" @click.stop>
										<el-image v-if="scope.row.value.substring(0,4)=='http'" preview-teleported
											:preview-src-list="[scope.row.value.split(',')[0]]"
											:src="scope.row.value.split(',')[0]" style="width:64px;height:64px;border-radius:14px"></el-image>
										<el-image v-else preview-teleported
											:preview-src-list="[$config.url+scope.row.value.split(',')[0]]"
											:src="$config.url+scope.row.value.split(',')[0]" style="width:64px;height:64px;border-radius:14px">
										</el-image>
									</div>
							...
							<el-table-column label="操作" class-name="operation-cell" width="260"  :resizable='true' :sortable='false' align="left" header-align="left">
								<template #default="scope">
									<el-button class="btn_secondary" type="info" v-if=" btnAuth('config','查看')" @click.stop="infoClick(scope.row.id)">详情</el-button>
									<el-button class="btn_secondary" type="primary" v-if=" btnAuth('config','修改')" @click.stop="editClick(scope.row.id,scope.row)">修改</el-button>
								</template>
							</el-table-column>
						</el-table>
					</div>
